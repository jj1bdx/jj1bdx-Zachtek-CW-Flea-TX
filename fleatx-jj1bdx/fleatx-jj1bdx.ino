// CW flea transmitter, Ver 1.5 Copyright 2021 Kevin Loughin, KB9RLW
...
//
// To Compile:
// USe the Menu "Sketch/Include Library menu to add the
// "Etherkit Si5351" library by Jason Mildrum.
//
// To Upload:
// Use the menu "Tools/Board:" and "Tools/Processor:" to set the board type to
// "Arduino Pro or Pro Mini" and processor to "ATMega328P (3.3V 8MHz)
//
// Original Version info:
//  1.3 First test version for Revision 2 of the hardware.
//  1.4 First version for the Revision 3 of the hardware
//  1.5 Changed Up and Down buttons as they had been switched
//      in the new hardware revision
...
#define FilterRelay                                                            \
  A2 // Output to Relay that will pick etiher 40m filter or 30/20m filter.  LOW=
     // 40m filter
#define TXRelay A3   // Output to RX/TX relay. LOW= RX
...
long Band40mbottom =
    700000000; // Lowest frequency of 40m Band CentiHertz. 100 CentiHertz=1Hz
...
long Band40mstart =
    701000000; // Start frequency of 40m Band CentiHertz. 100 CentiHertz=1Hz
...
uint32_t ReferenceFrequency =
    26000000; // TCXO Frequency used as a reference for the Si5351PLL
uint8_t CurrentBand =
    Band40m; // Keeps track on what band we are using right now. Start with 40m.
long Currentbandtop, Currentbandbottom; // Keeps tracks of the band edges of the
...
void setup() {
  ...
  pinMode(LED40m, OUTPUT); // These are outputs to turn on LEDs
  ...
  pinMode(BandButton,
          INPUT_PULLUP); // Set BandButton pin as input with pullup enabled
  ...
  Serial.print("CW Flea. Version 1.5");
  StartupLEDBlink();
  SetBand(CurrentBand); // Pick the correct Low Pass filter

  si5351.init(SI5351_CRYSTAL_LOAD_8PF, ReferenceFrequency,
              0); // initialize the VFO
...
// interrupt handler routine.  Handles the timing counters, runs 1000 times per
// second
SIGNAL(TIMER0_COMPA_vect) {
  ...
  if ((tunecount > 0) &&
      tunevfo) // If we're tuning, count down before increase tuning speed
  {
    tunecount--;
  }
}
...
void loop() {
  ...
  if (keyed &&
      !keyedprev) // did we just unkey? set the timeout value, turn off VFO
  {
    tail = taildefault;
    keyedprev = 1;
  ...
  if (buttonup && buttondn &&
      tuning) // if no buttons are pressed, reset tuning flag
  {
    tuning = 0;
  }
  ...
  if (tunevfo && !tunecount &&
      !tuning) // buttons released, delay done, turn off VFO
  {
    delay(100);
...
// Pick the next band
void NextBand() {
  CurrentBand++; // Advance state machine for what band is in use, if reached
                 // the last band -roll over to the lowest
  ...
  do {
    delay(10);
  } while (digitalRead(BandButton) ==
           Pressed); // Wait for the BandButton to be released
}
